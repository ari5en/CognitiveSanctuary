...
const appleEmoji = (cp) =>
  `https://emojicdn.elk.sh/${encodeURIComponent(cp)}?style=apple`;
...
const RESULT_LEVELS = [
  {
    ...
    codepoint: "😄",
    color: "#22c55e",
    desc: "Great work! You're in excellent shape.",
  },
  {
    max: 50,
    codepoint: "🙂",
    color: "#38bdf8",
    desc: "Moderate fatigue. Keep rest consistent.",
  },
  {
    max: 75,
    codepoint: "😐",
  ...
  },
  {
    ...
    codepoint: "😫",
    color: "#f43f5e",
    desc: "High burnout risk. Recovery mode activated.",
  },
];
  ...
  <ModalBase>
    <div className="text-center space-y-4">
      ...
      <p className="text-sm" style={{ color: "#6b7280" }}>
        Your progress will be saved and burnout will be evaluated.
      </p>
...
const SkipBreakModal = ({ onConfirm, onCancel }) => (
  <ModalBase>
    <div className="text-center space-y-4">
      ...
      <p className="text-sm" style={{ color: "#6b7280" }}>
        Skipping breaks increases burnout risk. This will be recorded.
      </p>
      <div className="flex gap-3 pt-2">
        <button
          onClick={onCancel}
  ...
  return (
    <ModalBase>
      ...
      <p className="text-sm mb-6" style={{ color: "#6b7280" }}>
        This helps calibrate your burnout score.
      </p>
...
const ResultScreen = ({ score, level, config, onContinue }) => {
  const lvl =
    RESULT_LEVELS.find((l) => score <= l.max) ||
  ...
  return (
    ...
    >
      <div
        className="rounded-3xl shadow-2xl p-12 max-w-sm w-full text-center space-y-6"
        style={{ background: "#F7F3EE" }}
      >
        <div className="flex justify-center">
          <img
            src={appleEmoji(lvl.codepoint)}
            alt={level}
            width={80}
            height={80}
            draggable={false}
        ...
        <div>
          ...
          <p className="text-5xl font-bold" style={{ color: lvl.color }}>
        ...
        {config && (
          <div
            className="rounded-2xl p-4 text-left space-y-1"
          ...
          >
            <p
              ...
              style={{ color: "#9ca3af" }}
            >
              Next Session (Adapted)
            </p>
            <p className="text-xs" style={{ color: "#374151" }}>
              Focus: <strong>{config.focusDuration} min</strong>
            </p>
            {/* <p className="text-xs" style={{ color: "#374151" }}>Break every: <strong>{config.breakInterval} min</strong></p> */}
            <p className="text-xs" style={{ color: "#374151" }}>
              Break duration: <strong>{config.breakDuration} min</strong>
            </p>
          </div>
        )}
        <button
...
const SessionsPage = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!target) {
          setLoadError(
            "No planned sessions. Generate one on the Schedule page.",
          );
          return;
        }
        ...
        if (plannerData && target.status === "Planned") {
          target.plannedFocusDuration =
            plannerData.plannedFocusDuration ??
            plannerData.planned_focus_duration ??
            target.plannedFocusDuration;
          target.plannedBreakDuration =
            plannerData.plannedBreakDuration ??
            plannerData.planned_break_duration ??
            target.plannedBreakDuration;
        }

        setSession(target);
        setPhase(Phase.Idle);
        const t = await getTasksBySession(
          target.sessionId || target.session_id,
        ).catch(() => []);
        setTasks(t);
      } catch {
  ...
  const handleMoodSubmit = async (mood) => {
    ...
    try {
      ...
      const data = await completeSession(sid, {
        ...
        endTime,
        studyDuration: +(focusElapsed / 60).toFixed(2),
        mood,
        breaksSkipped,
      });
      setResultData({
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 sm:px-0 flex flex-col h-full">
      {/* Modals */}
      <AnimatePresence>
        {showConfirmExit && (
          <ConfirmExitModal
        ...
        {showSkipBreak && (
          <SkipBreakModal
            onConfirm={handleSkipBreakConfirm}
            onCancel={() => setShowSkipBreak(false)}
          />
        )}
        {showMood && (
          <MoodModal onSubmit={handleMoodSubmit} isSubmitting={isSubmitting} />
        )}
      </AnimatePresence>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-7">
        <div className="min-w-0">
          ...
          <p
            className="flex items-center gap-1.5 text-sm font-medium mt-1 uppercase tracking-widest"
            style={{ color: "#9ca3af" }}
          >
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold transition-all disabled:opacity-30 w-full sm:w-auto"
          style={{
      ...
      <div className="w-full pb-10">
        {loadError ? (
          <div
            className="rounded-3xl p-10 text-center w-full"
            style={{ background: "#F7F3EE" }}
          >
          ...
          <div className="w-full flex flex-col lg:flex-row gap-4 items-start">
            {/* ── Timer card (light) ─────────────────────────────────────────── */}
            <div
              className="flex-1 w-full flex flex-col items-center gap-4 rounded-3xl py-6 sm:py-8 px-5 sm:px-10 relative overflow-hidden"
              style={{ background: "#F7F3EE" }}
            >
                    ...
                    <div className="relative w-44 h-44 sm:w-56 sm:h-56 mx-auto">
                      ...
                      >
                        ...
                        <circle
                          ...
                          r="80"
                          fill="none"
                          stroke={isFocusPhase ? "#15803d" : "#0284c7"}
                          strokeWidth="10"
                          strokeLinecap="round"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span
                          ...
                          style={{
                            color: isFocusPhase ? "#15803d" : "#0284c7",
                          }}
                        >
              ...
              {/* Stats strip */}
              <div className="relative z-10 flex flex-wrap gap-6 sm:gap-12 text-center mt-1 justify-center">
                <div>
                  <p
